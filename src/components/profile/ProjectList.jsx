...
export default function ProjectList({ projects }) {
  ...
  const months = [
    ...new Set(projects.map((project) => project.startedMonth)),
  ];
  ...
  const filtered = useMemo(() => {
    return projects.filter((project) => {
      ...
      const matchesMonth =
        month === 'All' || project.startedMonth === month;
      ...
      <div className="rounded-2xl border border-stone-200 bg-white p-3 shadow-sm sm:p-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-stone-200 bg-stone-50 py-3 pl-11 pr-4 text-sm font-medium text-stone-900 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:flex">
          ...
          <select
            ...
            className="rounded-xl border border-stone-200 bg-white px-3 py-2.5 text-sm font-bold text-stone-600 outline-none focus:border-emerald-500"
          >
            <option>All</option>
          ...
          <label className="sr-only" htmlFor="project-month">
            Filter by started month
          </label>
          ...
          {(query || status !== 'All' || month !== 'All') && (
            <button
              type="button"
              onClick={clearFilters}
              className="col-span-2 text-sm font-bold text-emerald-700 hover:text-emerald-800 sm:ml-auto"
            >
              Clear filters
            </button>
          )}
      ...
      {filtered.length ? (
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-stone-300 bg-white px-5 py-12 text-center">
          ...
          <p className="mt-1 text-sm text-stone-500">
            Try another name, status or month.
          </p>
        </div>
      )}
    </section>
